...
def get_image_list(data_root, split):
    ...
    filelist = list(glob(os.path.join(data_root, ('{}/*/*').format(split))))
...
class ljdataset(object):
    ...
    def wav2lip_crop_audio_window(self, spec, start_frame):
        ...
        if type(start_frame) == int:
            ...
            start_frame_num = self.get_frame_id(start_frame) - hps.T//2
    ...
    def get_segmented_mels_modified(self, spec, center_frame):
        ...
        for start_frame_num in window_ids:
            ...
            mels_indiv = np.asarray(mels_indiv)
            mels.append(mels_indiv)
    ...
    def __getitem__(self, idx):
        while 1:
            ...
            wav2lip_window = self.read_window_wav2lip(wav2lip_window_fnames, hps.wav2lip_img_size)
            if wav2lip_window is None:
                continue

            wrong_wav2lip_window = self.read_window_wav2lip(wrong_wav2lip_window_fnames, hps.wav2lip_img_size)
            ...
            wav2lip_mel = self.wav2lip_crop_audio_window(orig_mel, img_name)
            ...
            ## Wav2lip Items
            wav2lip_window = self.prepare_wav2lip_window(wav2lip_window)
            y = wav2lip_window.copy()
            wav2lip_window[:, :, :, wav2lip_window.shape[3]//2:] = 0.

            
            wrong_wav2lip_window = self.prepare_wav2lip_window(wrong_wav2lip_window)
            wav2lip_x = np.concatenate([wav2lip_window, wrong_wav2lip_window], axis=1) ## moving this so the channel concatenation is in the correct position
...
class ljcollate():
    ...
    def __call__(self, batch):
        ...
        wav2lip_inputs = torch.FloatTensor(np.asarray([x[3] for x in batch])).transpose(0,1)
        indiv_mels = torch.FloatTensor(np.asarray([x[4] for x in batch])).transpose(0,1).unsqueeze(3)
        wav2lip_mel = torch.FloatTensor(np.asarray([x[5] for x in batch])).unsqueeze(1)
        y_window = torch.FloatTensor(np.asarray([x[6] for x in batch])).transpose(0,1)
