...
class ljdataset(object):
    ...
    def __getitem__(self, idx):
        while 1:
            ...
            wav2lip_window_fnames = self.get_wav2lip_window(img_name)
            wrong_wav2lip_window_fnames = self.get_wav2lip_window(wrong_img_name)
            ...
            if (lip2wav_mel.shape[0] != hps.mel_step_size):
                idx = np.random.randint(len(self.all_videos))
                continue
            indiv_mels, start_frame_num = self.get_segmented_mels(orig_mel.copy(), img_name)
            ...
            wav2lip_x = np.concatenate([wav2lip_window, wrong_wav2lip_window], axis=0)
            ...
            return lip2wav_x, lip2wav_mel, ref , wav2lip_x, indiv_mels, wav2lip_mel, y, start_frame_num
...
class ljcollate():
    ...
    def __call__(self, batch):
        ...
        start_nums = torch.FloatTensor(np.asarray([x[7] for x in batch]))


        
        return inputs, input_lengths, mel_targets, targets_lengths, split_infos, embed_targets  \
        , wav2lip_inputs, indiv_mels, wav2lip_mel, y_window, start_nums
