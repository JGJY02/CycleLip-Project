...
from .layers import Conv3d, LinearNorm, ConvolutionEncoder, custom_conv1d, HighwayNet
...
class Attention(nn.Module):
    ...
    def get_alignment_energies(self, query, processed_memory,
        ...
        # print("self.attention_variable_projection device {}".format(self.attention_variable_projection.device))
        # print("self.attention_bias device {}".format(self.attention_bias.device))
        # print("processed_memory device {}".format(processed_memory.device))
        # print("attention_weights_cat device {}".format(processed_attention_weights.device))
        # print("processed_query device {}".format(processed_query.device))
        # end of addition
...
class Decoder(nn.Module):
    ...
    def inference(self, memory): # Removed gate outputs Jared
        ...
        mel_outputs, alignments = [], []
        while len(mel_outputs) < hps.num_mels/hps.outputs_per_step:
            ...
            decoder_input = mel_output.reshape(-1,hps.num_mels)[-1,:].unsqueeze(0)
...
class Tacotron2(nn.Module):
    ...
    def inference(self, inputs, speaker_embedding):
        # print(inputs.shape)
        frames = mode(inputs).float()
        speaker_embedding = mode(speaker_embedding).float()
        
        encoder_outputs = self.encoder.inference(frames)

        ## Attach speaker embedding for decoder
        tileable_shape = [-1, 1, hps.speaker_embedding_size]
        tileable_embed_targets = speaker_embedding.reshape(tileable_shape)
        # print(tileable_embed_targets.shape)
        tiled_embed_targets = tileable_embed_targets.repeat(1, encoder_outputs.shape[1], 1)
        encoder_outputs = torch.cat((encoder_outputs, tiled_embed_targets), dim=2)
        ## end of addition
        ...
        return [mel.transpose(1,0) for mel in outputs[1]], outputs[2]
